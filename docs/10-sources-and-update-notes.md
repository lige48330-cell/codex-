...
1. 保留 Windows 新手主线；在“第一次提问”页补充：优先执行低风险、可验证的任务，改动后审查 diff 并运行相关测试或验证命令。
2. 在下一步资源页补充 `AGENTS.md` 的官方指南，并强调项目规则应写清测试/构建命令、代码约定、不可触碰文件和危险操作的确认要求；不要堆砌泛泛而谈的提示词。
3. 保持 Skills、MCP、插件和子代理为进阶内容；继续采用“先看权限范围，再安装或授权”的安全原则，避免推荐一键脚本、代理、中转或要求提交密钥的方案。
4. 对登录方式、Windows 支持、版本、模型、套餐、命令参数和界面位置等时效信息，继续以本仓库已有的官方入口为准，避免把社区教程中的描述写成保证。
